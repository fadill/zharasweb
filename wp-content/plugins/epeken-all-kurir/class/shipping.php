...
*/
	...
	{	
		...
		public function init() {
					...
					add_action( 'woocommerce_cart_calculate_fees', array($this,'calculate_insurance'));	// fadill uncommented
	   ...
	   public function generate_enable_cod_html() {
		   ...
		   ?>
			...
			<td>
				...
				id = "woocommerce_epeken_enable_cod" />
			<td>
			</tr>
		   <?php
		   return ob_get_clean();
...
<div style="float: left;width: 100%; margin-bottom: 5px;">Diskon Tarif POS : <input type="number" placeholder="0" style="width: 80px;" name="epeken_diskon_tarif_pos"  value="<?php echo $epeken_diskon_tarif_pos; ?>"/>%</div>
			...
			</div><div style="float: left;width: 100%; margin-bottom: 5px;"><hr></div>
                        <p><div class="pilihan_kurir_div"><input name="enabled_rpx" id="enabled_rpx" type="checkbox" <?php if ($en_rpx === "on"){echo "checked";} ?>>RPX</input></div></p>
			...
			<div style="clear: left;">
                        <p>  
                        <div class="pilihan_kurir_div"><input name="enabled_jne_trucking_tarif" id = "enabled_jne_trucking_tarif" type="checkbox" <?php if ($en_enabled_jne_trucking === "on"){echo "checked";} ?>>JNE TRUCKING (Hanya untuk kota asal Jabodetabek)</input></div>
                        </p>    
                        </div>	
			<div style="clear: left;">
			<p>
			...
			<div style="clear: left;">
                        ...
                        <div style="clear: left;"><p><em>Isikan nilai subsidi ongkir yang ingin Anda berikan untuk pelanggan Anda. Jika Ongkir lebih dari nilai ini, maka Pelanggan akan mendapatkan potongan sejumlah yang Anda masukkan nilainya sebagai subsidi ongkir. Jika nilai ongkir kurang dari jumlah ini, maka ongkir akan gratis. Subsidi ongkir ini akan berlaku untuk semua kurir lokal dan tidak berlaku untuk kurir internasional</em></p>
						</div>
			</div>
		</td>
		</tr>
	...
	public function set_shipping_cost() {
			...
			if ($en_pos_kk === "on" || $en_pos_end === "on" || $en_pos_vg === "on") {
			 $weight = 1000;
			 $length = 0;
			 $width = 0;
			 $height = 0;
			...
			if($en_jtr_tarif === 'on') {
				...
				$logger -> add('epeken-all-kurir', $content_jtr_tarif);
				...
				if(!empty($content_jtr_tarif_decoded)){
			 	 ...
			 	 for($i=0; $i <= sizeof($content_jtr_tarif_decoded); $i++) {
					...
					if($package_name === 'JNE TRUCKING' && $this->bulatkan_berat($this->shipping_total_weight) < 50){
						continue;
					}
			...
			if($en_custom_tarif === 'on') {
				$logger = new WC_Logger();
				$content_custom_tarif = epeken_get_custom_tarif($this -> shipping_city, $this -> bulatkan_berat($this -> shipping_total_weight), $this->origin_city) ;
				$logger -> add('epeken-all-kurir', $content_custom_tarif);
	...
	public function is_shipping_exclude ($shipping_label) {
		...
		if ($shipping_label === 'JNE SPS' || $shipping_label === 'JNE CTCSPS' || $shipping_label === 'TIKI SDS' || $shipping_label === 'RPX SDP' || $shipping_label === 'JNE CTCBDO' || $shipping_label === 'JNE PELIK')  {
			...
			return $ret;
                ...
                $en_jne = get_option('epeken_enabled_jne'); $en_tiki = get_option('epeken_enabled_tiki'); 
	...
	public function add_subsidi_information() {
		...
		?>
		<tr>
			<td colspan=2>Anda mendapatkan subsidi ongkir sebesar Rp.<?php echo $subsidi;?></td>
		</tr>
		<?php
	}
	...
	public function count_cart_weight_and_dimension(){
		 ...
		 $this -> shipping_metric_dimension = 0;
			...
			$length=0;$width=0;$height=0;
					...
					$metric_dimension = $metric_dimension + ((intval($product_data -> get_length()) * intval($product_data -> get_width()) * intval ($product_data -> get_height())) * $value['quantity'] /6000);
					$length = $length + intval($product_data -> get_length());
					$width = $width + intval($product_data -> get_width());
					$height = $height + intval($product_data -> get_height());
                                }
    ...
    public function apply_subsidi ($rate) {
		...
		$logger -> add('epeken-all-kurir', 'subsidi ongkir : '.$epeken_subsidi_ongkir);
		$epeken_subsidi_min_purchase = get_option('epeken_subsidi_min_purchase');
		$logger -> add('epeken-all-kurir', 'minimum belanja subsidi : '.$epeken_subsidi_min_purchase);
		$total = $this -> get_cart_total();
		$logger -> add('epeken-all-kurir', 'total : '. $total);
	...
	public function epeken_is_applied_coupon_free_shipping() {
		...
		$tmp = ob_get_clean();
		$logger -> add('epeken-all-kurir', $tmp);
	...
	public function calculate_insurance() {
		 ...
		 if(is_numeric($this -> current_currency_rate) && $this -> current_currency_rate > 0) { 
				 ...
				 /*---- fadill added -----*/
				 $premium = 3000;	//fadill add custom kardus cost
                 $this -> insurance_premium = $premium;
                 $woocommerce->cart->add_fee( 'Biaya Packaging Kardus', $premium, true, '' );
				 /*---- end of edit -----*/				 
        }
