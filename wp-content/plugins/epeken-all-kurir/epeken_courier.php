...
$api_custom_tarif=EPEKEN_SERVER_URL.'/api/index.php/epeken_get_custom_tarif/';
...
$api_get_jne_trucking=EPEKEN_SERVER_URL.'/api/index.php/epeken_get_jne_trucking_tarif/';
$api_get_sap_express=EPEKEN_SERVER_URL.'/api/index.php/epeken_get_sap_express_tarif/';
...
if (in_array('woocommerce/woocommerce.php', apply_filters( 'active_plugins', get_option( 'active_plugins'))) || array_key_exists( 'woocommerce/woocommerce.php', maybe_unserialize( get_site_option( 'active_sitewide_plugins') ) )) {
	...
	function epeken_custom_checkout_fields( $fields ) {
		 ...
		 $shipping_address_1_tmp = $fields['shipping']['shipping_address_1'];
	  	 $billing_city_tmp = isset($fields['billing']['billing_city']) ? $fields['billing']['billing_city'] : null;
		 $shipping_city_tmp = isset($fields['shipping']['shipping_city']) ? $fields['billing']['billing_city'] : null;
		 ...
		 $fields['billing']['billing_address_2']['class'] = array(
                         'address-field','validate-required','update_totals_on_change');
		 ...
		 $fields['billing']['billing_state']['class'] = array('form-row-first','address_field','validate-required');
	      	 $fields['billing']['billing_postcode'] = $billing_postcode_tmp;
		 $fields['shipping']['shipping_state'] = $shipping_state_tmp;
	 	 $fields['shipping']['shipping_state']['class'] = array('form-row-first','address_field','validate-required');
	...
	function epeken_is_product_in_category($productid,$product_category_name){
	 ...
	 /* ---- fadill edit ---- */
		...
		add_action( 'woocommerce_before_checkout_billing_form', 'epeken_checkout_insurance_field' );
	 /* ---- end of edit ---- */
...
function epeken_add_konfirmasi_pembayaran_button($actions,$order) {
	...
	$order = new WC_Order($order_number);
		/*---- fadill added -----*/	
        if($order -> has_status( array( 'completed', 'cancelled', 'refunded', 'processing' ) ) )  // if order is cancelled or completed 
                return $actions;
		$user_email = $order->billing_email;
		$total_pembayaran = $order->get_total();
		/*---- end of edit -----*/
				
        $action['name'] = 'Konfirmasi Pembayaran';
        $action['url'] = get_permalink(get_page_by_title('Konfirmasi Pembayaran')).'?order_id='.$order_number.'&your-email='.$user_email.'&JumlahTransfer='.$total_pembayaran;
...
function epeken_submit_konfirmasi_pembayaran() {
	...
	if ($order == false) {
		echo "Order ".$order_id." tidak ditemukan";
		return;
	}
